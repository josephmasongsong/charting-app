...
export default function CreateEventForm() {
  ...
  // Form data state - ensure all values start as strings
  const [formData, setFormData] = useState({
    // Step 1
    title: '',
    eventDate: undefined as Date | undefined,
    description: '',
    // Step 2 - new structure for durations
    ...
    try {
      ...
      error.errors?.forEach((err: any) => {
        if (err.path.length > 0) {
          // Handle nested paths like "eventDuration.hours"
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const apiData = {
        ...validation.data,
        eventDate: validation.data.eventDate.toISOString(),
      };
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription className="space-y-2">
              <p>
                Please review your event details carefully. Once created, the
                event information cannot be edited.
              </p>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={handleSubmit} disabled={loading}>
              {loading ? 'Creating...' : 'Yes, Create Event'}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
