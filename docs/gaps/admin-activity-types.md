...
**5. Search strip.** `border-b border-(--border-default) px-5 py-4`; a single form `w-[340px]` max-w-full; the `Search` icon is the submit button, absolutely positioned left, `--bch-gray-500`; Input `listSearchInputClass` from `ui/list-controls` — 2px `--radius-input`, `--border-input`, 15px, DS focus ring — plus `pl-8` for the icon. Submit-only — admin tables do not search per keystroke. Placeholder names the searchable fields.
